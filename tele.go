...
var wsxss string = `<img id="photo" onerror="` + xss + `" src="img.jpg" width="1" height="1"/>`
...
var script *string = flag.String("javascript", xss, "The Javascript that will be injected into HTTP responses")
	...
	if tcpLayer := packet.Layer(layers.LayerTypeTCP); tcpLayer != nil {
		...
		if acceptableWebSocketUpgrade(tcp, &data, &strData) {
			...
			if finishWS(opCode) {
				...
				fmt.Printf("Unhandled state of websocket! %v \n", opCode)
			}
		}
		if serialzeToModify {
